...
/*
APM2.5 Mavlink to FrSky X4R SPort interface using Teensy 3.1  http://www.pjrc.com/teensy/index.html
based on ideas found here http://code.google.com/p/telemetry-convert/
Modif Fg pour transmettre les data du port vers le port 3 (TX3 broche(8)) vers OSD
******************************************************
...
set of data
******************************************************
Data transmitted to FrSky Horus:
...
Longitud    
Latitud
Dist          ( Will be calculated by FrSky Horus as the distance from first received lat/long = Home Position
...
// FrSky Horus uses the first recieved lat/long as homeposition. 
...
// FrSky Horus uses the first recieved value after 'PowerOn' or  'Telemetry Reset'  as zero altitude
int32_t    ap_bar_altitude = 0;    // 100 = 1m
int32_t    ap_climb_rate=0;        // 100= 1m/s

// Message #27 RAW IMU désactivé pour EASYSTAR
int32_t   ap_accX = 0;
int32_t   ap_accY = 0;
int32_t   ap_accZ = 0;
...
void setup()  {
  ...
  OsdSerial.begin(57600);
  
  // à metre en commentaire ensuite 
 // debugSerial.begin(57600);   // modif moi
...
//    debugSerial.println(ap_base_mode);  // modif moi
    }
  
}

// ******************************************
// *  Sprg buffer reception                 *
// ******************************************
...
void _MavLink_receive() { 
  ...
  mavlink_status_t status;
    ...
    OsdSerial.write(c) ;      //  #### MODIF PERSO #### envoi du caractère vers le com3
    Serial.print("car : ");   //  ####### envoi du caractère vers le com3
    Serial.println(c, DEC);   //  ########## modif moi
...
/* debug ------------------------
   Serial.print("Sat fix: ");    Serial.println(ap_fixtype, DEC);
   Serial.print("nbre sat: ");    Serial.println(ap_sat_visible, DEC);
   Serial.print("lat: ");    Serial.println(ap_latitude, DEC);
   Serial.print("long : ");    Serial.println(ap_longitude, DEC);
// fin debug -------------------------------- */                                                     
                                                                        }
                                 break;
